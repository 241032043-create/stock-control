...
const server = createServer(async (req, res) => {
  ...
  try {
    ...
    if (req.method === 'POST' && req.url === '/api/telegram/notify') { const input = await body(req); if (!input.text) return json(res, 400, { error: 'Notification text is required' }); const result = await sendTelegram(input.text); return json(res, 200, result); }
