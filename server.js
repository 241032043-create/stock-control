...
const splitTelegramText = (text, limit = 3900) => { const chunks = []; for (let offset = 0; offset < text.length; offset += limit) chunks.push(text.slice(offset, offset + limit)); return chunks; };
const sendTelegram = async (text) => { if (!telegramToken || !telegramChatId) return { sent: false, configured: false }; const chunks = splitTelegramText(String(text)), messages = []; for (const chunk of chunks) { const response = await fetch(`https://api.telegram.org/bot${telegramToken}/sendMessage`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ chat_id: telegramChatId, text: chunk }) }); const result = await response.json().catch(() => ({})); if (!response.ok || result.ok === false) throw new Error(result.description || `Telegram returned ${response.status}`); messages.push({ messageId: result.result?.message_id ?? null, chatType: result.result?.chat?.type ?? null, chatTitle: result.result?.chat?.title ?? null }); } return { sent: true, configured: true, parts: chunks.length, messages }; };
const serve = (req, res) => { const pathname = new URL(req.url, 'http://localhost').pathname; const requested = pathname === '/' ? 'index.html' : normalize(pathname.slice(1)); if (requested.startsWith('..')) return json(res, 403, { error: 'Forbidden' }); const file = join(root, requested); if (!existsSync(file)) return json(res, 404, { error: 'Not found' }); const types = { '.html': 'text/html', '.js': 'text/javascript', '.css': 'text/css', '.json': 'application/json', '.webmanifest': 'application/manifest+json', '.svg': 'image/svg+xml' }; const contentType = types[extname(file)] || 'application/octet-stream'; res.writeHead(200, { 'Content-Type': contentType, 'Cache-Control': contentType.startsWith('text/html') || contentType.includes('javascript') || contentType.includes('css') ? 'no-store, no-cache, must-revalidate' : 'public, max-age=3600' }); res.end(readFileSync(file)); };
const server = createServer(async (req, res) => {
  ...
  try {
    ...
    if (req.method === 'GET' && req.url === '/api/state') return json(res, 200, { products: data.products, movements: data.movements, priceHistory: data.priceHistory || [] });
